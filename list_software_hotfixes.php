...
if ($sub <> "sw1"){
  ...
  $sql = "SELECT count(software_name) AS software_count, software_name, software_version, software_publisher FROM software where (software_name LIKE '%hotfix%' OR software_name LIKE '%update%' OR software_name LIKE '%Service Pack%') GROUP BY software_name, software_version ORDER BY " . $sort . ", software_name, software_version";
  $result = mysql_query($sql);
  ...
  if ($myrow = mysql_fetch_array($result)){
    ...
    do {
      ...
      echo "  <td align=\"center\"><a href=\"" . $_SERVER["PHP_SELF"] . "?sub=sw1&amp;name=" . url_clean($myrow["software_name"]) . "&amp;version=" . url_clean($myrow["software_version"]) . "\" title=\"Show All Systems with this software\">" . $myrow["software_count"] . "</a></td>\n";
      echo "  <td><a href=\"" . $_SERVER["PHP_SELF"] . "?sub=sw1&amp;name=" . url_clean($myrow["software_name"]) . "\" title=\"Show All Systems with this software\">" . $myrow["software_name"] . "</a>&nbsp;&nbsp;</td>\n";
