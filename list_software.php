...
if ($sub == "sw1"){
  ...
  if ($software_version <> ""){
    $sql = "SELECT software_name, software_version, software_publisher, net_ip_address, system_uuid, system_name, system_description FROM software, system where software_name = '$name' AND software_version = '$software_version' AND software_uuid = system_uuid AND software_timestamp = system_timestamp ORDER BY " . $sort;
  } else {
  ...
  if ($myrow = mysql_fetch_array($result)){
    ...
    echo " <tr>";
    echo "  <td>&nbsp;&nbsp;<a href=\"list_software.php?sub=sw1&amp;name=" . url_clean($_GET["name"]) . "&amp;version=" . url_clean($software_version) . "&amp;sort=net_ip_address\">$l_ipa</a></td>\n";
    echo "  <td>&nbsp;&nbsp;<a href=\"list_software.php?sub=sw1&amp;name=" . url_clean($_GET["name"]) . "&amp;version=" . url_clean($software_version) . "&amp;sort=system_name\">$l_nam</a></td>\n";
    echo "  <td>&nbsp;&nbsp;<a href=\"list_software.php?sub=sw1&amp;name=" . url_clean($_GET["name"]) . "&amp;version=" . url_clean($software_version) . "&amp;sort=system_description\">$l_des</a></td>";
    if ($software_version == ""){
      echo "<td>&nbsp;&nbsp;<a href=\"list_software.php?sub=sw1&amp;name=" . url_clean($_GET["name"]) . "&amp;version=" . url_clean($software_version) . "&amp;sort=software_version\">$l_ver</a></td>";
    } else {}
    echo "\n </tr>\n";
    do {
      $bgcolor = change_row_color($bgcolor,$bg1,$bg2);
