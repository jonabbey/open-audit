...
$linux_soft_audit   = ( $_POST['check_linux_software']      == "true"    ) ? '1' : '0';
$windows_soft_audit = ( $_POST['check_windows_software']    == "true"    ) ? '1' : '0';
$linux_soft_list    = ( $_POST['check_linux_software_list'] == "true"    ) ? '1' : '0';
$nmap_srv           = ( $_POST['check_nmap_srv']            == "true"    ) ? '1' : '0';
$nmap_udp           = ( $_POST['check_nmap_udp']            == "true"    ) ? '1' : '0';
$nmap_tcp_syn       = ( $_POST['check_nmap_tcp_syn']        == "true"    ) ? '1' : '0';
...
$log_enable         = ( $_POST['check_log_enable']          == "true"    ) ? '1' : '0';
$cmd_interact       = ( $_POST['check_command_interact']    == "true"    ) ? '1' : '0';
$audit_local        = ( $_POST['check_cred_local']          == "true"    ) ? '1' : '0';

/* The following options are a must */
if ( empty($_POST['input_name']) ) { $error_list .= "<li>You must name the configuration<br>"; }
else {
  mysql_connect($mysql_server,$mysql_user,$mysql_password) or die("Could not connect to DB<br>");
  mysql_select_db($mysql_database);
  $sql = "SELECT * FROM audit_configurations WHERE audit_cfg_name LIKE '".$_POST['input_name']."'";
  $result = mysql_query($sql);
  if ( !@mysql_num_rows( $result ) < 1 AND $_POST['form_action'] != "edit" ) {
    $error_list .= "<li>That configuration name already exists.<br>";
  }
}
  
if ( $_POST['select_action'] == 'nothing' ) { $error_list .= "<li>You must pick an action (NMAP or PC audit)<br>";        }
if ( $_POST['select_audit']  == 'nothing' ) { $error_list .= "<li>You must pick an audit type (domain, list, range)<br>"; }

if ( !preg_match("/^[1-9]([0-9]+)?$/",$_POST['input_max_audits']) ) {
  $error_list .= "<li>Simultaneous audits must not be blank and must be a number (No leading zeros)<br>";
}

if ( !preg_match("/^[1-9]([0-9]+)?$/",$_POST['input_wait_time']) ) {
  $error_list .= "<li>Audit script wait time must not be blank and must be a number (No leading zeros)<br>";
}
else {
...
if ( $_POST['select_action'] == 'pc' or $_POST['select_action'] == 'command' ) {
  if ( $_POST['select_os'] == 'nothing' ) { $error_list .= "<li>You must choose an OS type for PC audits/Commands<br>"; }
  if ( $_POST['select_os'] == 'windows' && !preg_match("/^\/\/.*\.vbs$/i",$_POST['input_vbs']) ) {
    $error_list .= "<li>Audit.vbs path needs to be in the form of \"//server/share/audit.vbs\"<br>"; }
  if ( $_POST['select_audit_cred'] == "nothing" ) {
    if ( empty($_POST['input_cred_user']) ) { $error_list .= "<li>Audit username cannot be blank<br>"; }
    if ( empty($_POST['input_cred_pass']) ) { $error_list .= "<li>Audit password cannot be blank<br>"; }
  }
}
...
if ( $_POST['select_audit'] == 'iprange' ) {
  ...
  if ( !preg_match("/^(([1-9]?[0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]).){3}([1-9]?[0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$/", $ip_start) ) {
    $error_list .= "<li>Starting IP Address not valid<br>";
  }
  elseif ( $_POST['start_ip_4'] > $ip_end ) {
    $error_list .= "<li>The last octet of the ending IP needs to be greater than the last octect for the starting IP<br>"; 
  }
}
...
if ( isset($error_list) ) {
  ...
  if ( isset($_POST['del_query']) ) {
    foreach ( $_POST['del_query'] as $del_id ) {
      mysql_query("DELETE FROM `audit_mysql_query` WHERE audit_mysql_id = '{$del_id}'");
    }
  }
  ...
  if ( isset($_POST['query_fields_add']) ) {
    ...
    foreach ( $_POST['query_fields_add'] as $query_row ) {
      ...
      mysql_query("INSERT INTO `audit_mysql_query` (audit_mysql_table, audit_mysql_field, audit_mysql_sort, audit_mysql_data) 
                   VALUES('{$values[0]}','{$values[1]}','{$values[2]}','{$_POST['query_data_add'][$count]}')");
      array_push($query_ids,mysql_insert_id());
  ...
  if ( isset($_POST['query_fields_mod']) ) {
    ...
    foreach ( $_POST['query_fields_mod'] as $query_row ) {
      $values = explode(',',$query_row);
      mysql_query("UPDATE `audit_mysql_query` SET audit_mysql_table='{$values[1]}', audit_mysql_field='{$values[2]}',
                          audit_mysql_sort='{$values[3]}', audit_mysql_data='{$_POST['query_data_mod'][$count]}'
                   WHERE audit_mysql_id='{$values[0]}'");
  ...
  if ( $_POST['form_action'] == "edit" ) {
    $sql = "UPDATE audit_configurations SET 
              ...
              audit_cfg_mysql_ids='".@implode(",",$query_ids)."',
    ...
    $sql = "INSERT INTO audit_configurations ( audit_cfg_name, audit_cfg_action,
              ...
              audit_cfg_mysql_ids, audit_cfg_command_interact,
  ...
  mysql_query($sql) or die("Could not add config: " . mysql_error() . "<br>");
  /* Update the schedules only if this is an edit and it was updated successfully */
  if ( $_POST['form_action'] == 'edit' ) {
    mysql_query("UPDATE audit_schedules SET audit_schd_updated='1' WHERE audit_schd_cfg_id='{$_POST['config_id']}'");
  }
  ...
  if ( isset($_POST['query_fields_add']) ) {
    foreach ( $new_ids as $id ) {
      $values = explode(',',$query_row);
      mysql_query("UPDATE `audit_mysql_query` SET audit_mysql_cfg_id='{$config_id}' WHERE audit_mysql_id='{$id}'");
    }
  }

  $form_action = ( $_POST['form_action'] == "edit" ) ? 'updated' : 'added';
  echo "<div class=\"formResult\"> <img src=\"images/button_success.png\"/>
        <strong>The configuration has been {$form_action}</strong></div>";
}
?>
